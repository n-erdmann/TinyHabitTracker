...
class HabitMaster(Habit):
    ...
    def delete(self, name: str) -> tuple:
        ...
        :return: a tuple of three return messages
        """
    ...
    def deactivate(self, name: str, end: str) -> str:
        """Deactivate a habit starting from a given date.
        All data remains, but end date is updated
        :param name: habit name
    ...
    def reschedule(self, name: str, description: str, periodicity: str, start: datetime.date, frequency: int,
                   is_quan: int,
...
class HabitLog(Habit):
    ...
    def set_streak(self, name: str, log_date: datetime.date, rowid) -> tuple:
        ...
        :param rowid: id of the currently logged habit completion
        :return: tuple (string with information about successful DB update, current streak value for current log)"""
        streak = 0
        ...
        # fill the log period and previous ("last") period dynamically depending on periodicity:
        if periodicity == 'daily':
        ...
        if log_per == cur_per and str(log_year) == cur_year and cur_per != streak_per and log_date >= max_log_date:
            ...
            # new period has started -> previous period had a streak, continue counting up checking for the new period
            completion_count = dba.get_habit_completion_by_single_per(name=name, log_per=log_per)
            if completion_count >= frequency:
                cur_streak += 1
                streak_per_n = log_per
            ...
            # end previous streak one day before streak date and start new streak record,
            # beginning on log_date (initial)
            val_to = log_date - timedelta(days=1)
            valid_from = log_date
            dba.reset_streak(id=streak_id, name=name, val_to=val_to, val_from=log_date, cur_per=log_per,
                             cur_year=str(log_year))
            ...
            # create the list of all relevant periods
            ...
            for row in streak_base.iterrows():
                # check if in each line the count is >= the target frequency, if so streak += 1
                index, series = row
                ...
                if logged_freq >= frequency:  # int(row[1][1]) >= frequency:
                    ...
                    rowid_log = series.get('id')
                    logged_streak = series.get('cur_streak')
                    no_change = 0
                    # update log if cur_streak <> logged cur_streak!
                    if cur_streak != logged_streak:     # and streak_date_n != str(max_log_date):
                        self.dba.update_log(name=name, rowid=rowid_log, streak=cur_streak)
                        no_update = True
                else:  # habit broken
                    # first, create the streak record with last streak value and end date before starting from 0
                    if cur_streak > 0:  # no need to create a new record if it is still 0
                        ...
                        cur_streak = 0
                        self.dba.update_log(name=name, rowid=rowid, streak=cur_streak)
                        no_change = 1
        ...
        return return_message, cur_streak
    ...
    def log_completion(self, name: str, log_date: datetime.date, completed: int, quantity: float = '',
        ...
        # get periodicity of habit to fill log_per with the respective info:
        dba = DBActions()
